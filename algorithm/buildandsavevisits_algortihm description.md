...
It uses two parameters to aggregate detections:
    - <u>Intra-visit delay:</u> expressed in seconds, this parameter qualify the max delay between detections so they could be aggregated in the same visit.
...
Considered readings can be filtered using the following parameters:
    - <u>target:</u> targeted animal’s UHF ID number
    - <u>readers:</u> filter readings collected by the selected smartcase readers
    - <u>antennas:</u> filter readings collected by the selected antennas from the readers provided in parameter “readers”
